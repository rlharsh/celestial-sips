...
function pxlui_group(_x, _y, _array, _config = {}, _elementid = 0) : pxlui_element(_x,_y,_config,_elementid = 0) constructor{
	...
	on_drawGUI(function(){
		if (sprite_index != noone){
			...
			draw_sprite_ext(sprite_index,image_index,round(x+xoffset+xalign+_sprite_xoffset),round(y+yoffset+yalign+_sprite_yoffset),image_xscale,image_yscale,angle,color,alpha);
		}
	})
}
...
function pxlui_text(_x,_y,_text,_config = {}, _elementid = 0) : pxlui_element(_x,_y,_config,_elementid) constructor{
	...
	on_drawGUI(function(){
		if (text != -1){
			...
			draw_text_ext_transformed(round(x+xoffset+text_xoffset),round(y+yoffset+text_yoffset),text,-1,width,xscale,yscale,angle);
...
function pxlui_button(_x,_y, _text = "button", _callback = function(){pxlui_log("PXLUI: empty button has been pressed!");}, _config = {}, _elementid = 0) : pxlui_element(_x,_y,_config,_elementid) constructor{
	...
	hover_index = _config[$ "hover_index"] ?? 0;
	...
	on_drawGUI(function(){
		...
		draw_sprite_ext(sprite_index,s_index,round(x+xoffset+xalign+_sprite_xoffset*_sprite_ow),round(y+yoffset+yalign+_sprite_yoffset*_sprite_oh), _sprite_w,_sprite_h,angle,color,alpha);
		
		if (text != -1){
			draw_set_halign(halign);
			draw_set_valign(valign);
			draw_set_color(t_colour);
			//draw_text_ext_transformed(round(x+xoffset+text_xoffset),round(y+yoffset+text_yoffset),text,-1,width,1,1,angle);
			draw_text(round(x+xoffset+text_xoffset),round(y+yoffset+text_yoffset), text);
			draw_set_color(o_colour);
			draw_set_halign(fa_left);
			draw_set_valign(fa_bottom);
		}
	});
...
function pxlui_checkbox(_x,_y, _variable_array = [self,"toggle"], _config = {}, _elementid = 0) : pxlui_button(_x,_y,"",,_config,_elementid) constructor{
	...
	on_drawGUI(function(){
		...
		draw_sprite_ext(sprite_index,toggle,round(x+xoffset+xalign+_sprite_xoffset*_sprite_ow),round(y+yoffset+yalign+_sprite_yoffset*_sprite_oh), _sprite_w,_sprite_h,angle,color,alpha);
	});
}
